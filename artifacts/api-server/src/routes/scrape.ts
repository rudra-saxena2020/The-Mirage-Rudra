...
router.post("/scrape", async (req, res) => {
  ...
  try {
    ...
    req.log.error({ err: error }, "Scraping error");
    res.status(500).json({
      error: error.message || "Failed to render and scrape the URL.",
      details:
        "The site might be blocking browser-based requests or the URL is invalid.",
    });
  } finally {
    if (browser) {
      try {
        ...
        req.log.error({ err: e }, "Error closing browser");
      }
    }
  }
});
...
router.get("/proxy-image", async (req, res) => {
  ...
  try {
    ...
    req.log.error({ err: error }, "Proxy error");
